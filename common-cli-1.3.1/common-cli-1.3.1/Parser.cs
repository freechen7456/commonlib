...
public abstract class Parser : CommandLineParser
{
    ...
    private IDictionary<object, object> requiredOptions;

    protected void setOptions(Options options)
    {
        this.options = options;
        //this.requiredOptions = new List<object>(options.getRequiredOptions());
        this.requiredOptions = new Dictionary<object, object>();
        foreach (object o in options.getRequiredOptions())
        {
            if (o is Option)
            {
                requiredOptions[((Option)o).getKey()] = o;
            }
            else
            {
                requiredOptions[o] = o;
    ...
    protected IDictionary<object, object> getRequiredOptions()
    {
    ...
    public CommandLine parse(Options options, string[] arguments, Dictionary<string, string> properties, bool stopAtNonOption)
    {
        ...
        IList<string> tokenList = new List<string>(flatten(getOptions(), arguments, stopAtNonOption));

        IListIterator<string> iterator = tokenList.listIterator();
     ...
     */
    protected void processProperties(Dictionary<string, string> properties)
    {
        if (properties == null)
        {
            return;
            ...
            if (!cmd.hasOption(option) && !selected)
            {
                ...
                if (opt.hasArg())
                {
                    if (opt.getValues() == null || opt.getValues().Length == 0)
                    {
                        ...
                        catch (ApplicationException exp) //NOPMD
                        {
    ...
    protected void checkRequiredOptions()
    {
        ...
        {
            throw new MissingOptionException(getRequiredOptions().Values);
        }
    }
     ...
     */
    public void processArgs(Option opt, IListIterator<string> iter)
    {
     ...
     */
    internal protected void processOption(string arg, IListIterator<string> iter)
    {
